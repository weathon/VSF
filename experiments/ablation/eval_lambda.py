...
for folder in ["results_no_bias"]:
    ...
    for run in tqdm.tqdm(os.listdir(folder)):
        images = os.listdir(f"{folder}/{run}")
        ...
        with open(f"{folder}/" + run + "/vqa.json", "w") as f:
            json.dump(data, f)
